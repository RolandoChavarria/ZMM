...
*&      Form  main
*&---------------------------------------------------------------------*
FORM  main_pen USING p_lifnr    TYPE lifnr
                 p_ekgrp    TYPE bkgrp
                 p_proye    TYPE everk
                 p_folio    TYPE ihrez
...
* Validar datos
  PERFORM prerequest_pen USING p_test CHANGING e_subrc e_bapi_msg.
  CHECK e_subrc = 0.
* Lleando de tables aux
  PERFORM fill_aux_tables_pen USING t_items t_servi
...
* Llenado de datos a tabals structuras
  PERFORM fill_tables_pen USING p_lifnr p_ekgrp p_proye p_folio
        t_items t_servi CHANGING e_subrc e_bapi_msg.
  CHECK e_subrc = 0.
...
*&      Form  llenado_datos
*&---------------------------------------------------------------------*
FORM fill_tables_pen USING p_lifnr TYPE lifnr
                       p_ekgrp TYPE bkgrp
                       p_proye TYPE everk
                       p_folio TYPE ihrez
                       t_items TYPE zmmfm_0070_azul_tt
...
* Filling header structures
  PERFORM fill_header_pen USING p_lifnr p_ekgrp p_folio.
  CHECK e_subrc = 0.

  PERFORM fill_items_pen USING p_proye t_items t_servi
        CHANGING e_subrc e_bapi_msg.
...
*&---------------------------------------------------------.------------*
FORM fill_header_pen USING p_lifnr TYPE lifnr
                       p_ekgrp TYPE bkgrp
                       p_folio TYPE ihrez.
* Header structure
  gs_poheader-comp_code      = g_bukrs.                     "5030
  gs_poheader-doc_type       = g_bsart. "ZPED should be
...
*&---------------------------------------------------------------------*
FORM fill_aux_tables_pen USING t_items TYPE zmmfm_0070_azul_tt
                           t_servi TYPE zmmfm_0070_roja_tt
                  CHANGING e_subrc    TYPE subrc
                         e_bapi_msg TYPE bapi_msg.
    ...
    IF t_items[] IS NOT INITIAL.
      SELECT f~asnum f~asktx p~matkl p~bklas
        INTO TABLE it_asmdt FROM asmdt AS f
         INNER JOIN asmd AS p ON f~asnum = p~asnum
            FOR ALL ENTRIES IN t_items ""lt_asmdt
                    WHERE p~matkl EQ t_items-matkl AND
                  f~spras EQ sy-langu.
    ENDIF.
  ENDIF.
...
*&      Form  item_blue
*&---------------------------------------------------------------------*
FORM item_blue_pen USING u_item TYPE ty_item.
  DATA: l_t023t TYPE t023t.
*gs_bapiesllc-pckg_no         = '0000000001'.
  READ TABLE it_t023t INTO l_t023t
          WITH KEY matkl = u_item-matkl.
  CONCATENATE u_item-matkl l_t023t-wgbez60
    INTO l_t023t-wgbez60 SEPARATED BY space.
...
*&      Form  item_blue
*&---------------------------------------------------------------------*
FORM item_sche_pen USING u_item TYPE ty_item.
  DATA: lday(2) TYPE c,
        lmon(2) TYPE c,
        lyea(4) TYPE c.
...
*&      Form  item_acco
*&---------------------------------------------------------------------*
FORM item_acco_pen USING u_item TYPE ty_item
...
* Se busca la partida azul, pero en realidad se debe buscar la partida Roja, se converva si cambian de opinion
  READ TABLE it_asmdt INTO l_asmd WITH KEY matkl = u_item-matkl.
...
*  READ TABLE t_prmtrs INTO l_prmtrs WITH KEY zfield = l_asmd-bklas.
  READ TABLE t_prmtrs INTO l_prmtrs WITH KEY zfield = 'CUENTA'.
...
*&      Form  item_cond
*&---------------------------------------------------------------------*
FORM item_cond_pen USING u_item TYPE ty_item.
* Conditions
...
*&---------------------------------------------------------------------*
FORM fill_items_pen USING p_proye TYPE everk
                      t_items TYPE zmmfm_0070_azul_tt
                      t_rojas TYPE zmmfm_0070_roja_tt
             CHANGING e_subrc TYPE subrc
                      e_bapi_msg  TYPE bapi_msg.
...
* loop for items or partidas azules
  ...
  LOOP AT t_items INTO l_item.
    ...
    ENDIF.
* Structures
    PERFORM item_blue_pen USING l_item.
    PERFORM item_sche_pen USING l_item.
    PERFORM item_acco_pen USING l_item t_rojas.
    PERFORM item_cond_pen USING l_item.
* For each services
    PERFORM item_serv_pen USING l_item t_rojas.
...
*&      Form  validaciones
*&---------------------------------------------------------------------*
FORM prerequest_pen USING    p_test     TYPE bapiflag-bapiflag
                CHANGING e_subrc    TYPE subrc
                         e_bapi_msg TYPE bapi_msg.
  g_test = p_test.
  DATA: vtax        TYPE c LENGTH 2 .
  DATA: l_asmd   TYPE ty_asmdt.

  FIELD-SYMBOLS: <fp> TYPE ty_prmtrs.
* Clear varaibles
  CLEAR: g_bukrs, g_bsart, g_elpep, vnum18, g_ekorg,g_etmen,
        g_meins, g_matkl,g_matnr, g_bus_area,
...
* Get constants to the program
  SELECT zfield zchar INTO TABLE t_prmtrs  FROM zutil_parameters
         WHERE zreport EQ 'ZMMFM_0020_PENALIZACION_PO'.
  IF sy-subrc IS NOT INITIAL.
    e_subrc = '02'.
...
*-* Inicializa Variables
  ...
  IF sy-subrc EQ 0.
    vtax = <fp>-zchar.
  ENDIF.
  READ TABLE t_prmtrs ASSIGNING <fp> WITH KEY zfield = 'CUENTA'.
  IF sy-subrc EQ 0.
    l_asmd-bklas = <fp>-zchar.
  ENDIF.
ENDFORM.                    "prerequest
...
*&      Form  item_serv
*&---------------------------------------------------------------------*
FORM item_serv_pen USING u_item TYPE ty_item
                     t_rojas TYPE zmmfm_0070_roja_tt.
