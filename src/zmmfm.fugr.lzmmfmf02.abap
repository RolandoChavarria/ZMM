...
*&---------------------------------------------------------------------*
FORM  main_decrementa USING p_lifnr    TYPE lifnr
                 p_ekgrp    TYPE bkgrp
                 p_proye    TYPE everk
                 p_folio    TYPE ihrez
...
* Validar datos
  PERFORM prereq_decre USING p_test p_lifnr p_proye
                       CHANGING e_subrc e_bapi_msg.

  CHECK e_subrc = 0.
* Lleando de tables aux
  PERFORM fill_aux_tables_decre USING t_items
        CHANGING e_subrc e_bapi_msg.
  CHECK e_subrc = 0.
...
* Llenado de datos a tabals structuras
  PERFORM fill_tables_decre USING p_lifnr p_ekgrp p_proye
                                  p_folio t_items
                            CHANGING e_subrc e_bapi_msg.
...
*&      Form  llenado_datos
*&---------------------------------------------------------------------*
FORM fill_tables_decre USING p_lifnr TYPE lifnr
                       p_ekgrp TYPE bkgrp
                       p_proye TYPE everk
                       p_folio TYPE ihrez
                       t_items TYPE zmmfm_0070_azul_tt
...
* Filling header structures
  PERFORM fill_header_decre USING p_lifnr p_ekgrp p_folio.

  PERFORM fill_items_decre USING p_proye t_items
                           CHANGING e_subrc e_bapi_msg.
...
*&---------------------------------------------------------.------------*
FORM fill_header_decre USING p_lifnr TYPE lifnr
                       p_ekgrp TYPE bkgrp
                       p_folio TYPE ihrez.
* Header structure
...
*&---------------------------------------------------------------------*
FORM fill_items_decre USING p_proye TYPE everk
                      t_items TYPE zmmfm_0070_azul_tt
             CHANGING e_subrc TYPE subrc
                      e_bapi_msg  TYPE bapi_msg.
...
* loop for items or partidas azules
  LOOP AT t_items INTO l_item.
    ...
    ENDIF.
* Structures
    PERFORM item_blue_decre USING l_item.
    PERFORM item_sche_decre USING l_item.
    PERFORM item_acco_decre USING l_item.
    PERFORM item_cond_decre USING l_item.
...
*&      Form  item_acco
*&---------------------------------------------------------------------*
FORM item_acco_decre USING u_item TYPE ty_item.
...
*&      Form  item_cond
*&---------------------------------------------------------------------*
FORM item_cond_decre USING u_item TYPE ty_item.
* Conditions
...
*&      Form  item_blue
*&---------------------------------------------------------------------*
FORM item_sche_decre USING u_item TYPE ty_item.
  DATA: lday(2) TYPE c,
        lmon(2) TYPE c,
        lyea(4) TYPE c.
...
*&      Form  item_blue
*&---------------------------------------------------------------------*
FORM item_blue_decre USING u_item TYPE ty_item.
...
*&---------------------------------------------------------------------*
FORM fill_aux_tables_decre USING t_items TYPE zmmfm_0070_azul_tt
                CHANGING e_subrc    TYPE subrc
                         e_bapi_msg TYPE bapi_msg.
...
*&      Form  validaciones
*&---------------------------------------------------------------------*
FORM prereq_decre USING  p_test     TYPE bapiflag-bapiflag
                         p_lifnr    TYPE lifnr
                         p_proye TYPE everk
                CHANGING e_subrc    TYPE subrc
                         e_bapi_msg TYPE bapi_msg.
