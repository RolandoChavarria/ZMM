FUNCTION zbapi_po_getitems.                                 "#EC ENHOK
*"----------------------------------------------------------------------
*"*"Interfase local
*"  IMPORTING
...
*"      IMP_DOC_TYPES TYPE  WRF_PBAS_ESART_RTTY
*"----------------------------------------------------------------------
...
* fill ranges for the select
  PERFORM fill_rangesz TABLES r_ebeln r_bsart r_bedat r_ekgrp r_ekorg
...
*>>> Ajuste para soportar busqueda de mas de 1 tipo de PO >>>
  APPEND LINES OF imp_doc_types TO r_bsart.
  SORT r_bsart BY sign option low high.
  DELETE ADJACENT DUPLICATES FROM r_bsart COMPARING ALL FIELDS.
*<<< Ajuste para soportar busquedas de mas de 1 tipo de PO <<<
...
***                       and   loekz eq space  "Filtrar mark para borrar
...
* read po items without header data
  ELSE.
    ...
    SELECT * FROM ekpo APPENDING TABLE sekpo
                       WHERE bstyp EQ 'F'
      and   loekz eq space   "Filtrar marcados para borrar
                       AND   ematn IN r_ematn
                       AND   matnr IN r_matnr
                       AND   matkl IN r_matkl
                       AND   pstyp IN r_pstyp
